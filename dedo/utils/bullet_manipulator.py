...
class BaseManipulator:
    ...
    def set_movable(self, base_pos = np.array([0, 0, 0])):
        self.fixed = False
        self.moving = False
    ...
    def move_to(self, next_base_pos: np.array, next_base_ori: np.array) -> None:
        self.sim.changeConstraint(self.cid, next_base_pos, next_base_ori,
                                  maxForce=BaseManipulator.MAX_FORCE)
...
class BulletManipulator:
    ...
    def __init__(self, sim: bclient.BulletClient, robot_desc_file: os.path, ee_joint_name: str, ee_link_name: str,
                 control_mode: str, base_pos: np.array, base_quat, global_scaling: float,
                 rest_arm_qpos: np.array = None,
                 left_ee_joint_name: str = None, left_ee_link_name: str = None,
                 left_fing_link_prefix: str = None, left_joint_suffix: str = None,
                 left_rest_arm_qpos: np.array = None,
                 use_fixed_base: bool = False, kp: float = 0.1, kd: float = 1.0, min_z: float = 0.0, debug: bool = False):
        ...
        self.base = BaseManipulator(sim, self.info.robot_id)
        if not use_fixed_base: self.base.set_movable(base_pos)
        ...
        if rest_arm_qpos is not None:
            print("len(self.info.arm_jids_lst) =", len(self.info.arm_jids_lst))
        ...
        if left_rest_arm_qpos is not None:
            print("len(self.info.left_arm_jids_lst) =", len(self.info.left_arm_jids_lst))
    ...
    def load_robot(self, robot_path, ee_joint_name, ee_link_name,
        ...
        print("HEY pybullet.getNumJoints(robot_id)", pybullet.getNumJoints(robot_id))
    ...
    def move_base(self, lin_vel, rot_vel, dt=1./240):
        ...
        assert(self.base.cid is not None)
        base_state = self.sim.getLinkState(
            self.info.robot_id, 0, computeLinkVelocity=0)
        base_pos = base_state[0]
        base_quat = base_state[1]
        ...
        self.base.move_to(next_base_pos, next_base_ori)
        return
